...
class Model:
    ...
    def AICc(self, Y, pred_Y, k):
        Q = np.sum((np.array(Y) - np.array(pred_Y)) ** 2)
        n = len(Y)
        lnL = -n / 2 * (np.log(2 * np.pi * Q / n) + 1)
        aic = 2 * k * (n/(n - k - 1)) - 2 * lnL
        return aic
    ...
    def predict_NRLCQ(self, selectIDX, bicIDX):
        ...
        xic = self.AICc(NRLCQ_real_samp, NRLCQ_fit_samp, 2)
...
def traversalAllRange():
    ...
    for i in range(0,11):
        for j in range(0, 11):
            ...
            model.setCsvResultsName("这个是筛选之后的AICc国家的运行数据.csv")
...
def traversalOneRange(wight, dramatic):
    ...
    model.setCsvResultsName("这个是筛选之后的国家的运行数据.csv")
    countlist = [i for i in range(8)]
    print("----------------------------------------------------------------------------------------")
