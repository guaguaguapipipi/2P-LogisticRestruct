...
class Model:
    ...
    def __init__(self, filename, predlen):
        ...
        popt = self._countryarr[0].split("&")[0].split("=")[1].split("-")
        for i in popt:
            self._oripopt.append(float(i))
        clist = []
        idx = 0
        # 这里是初始化确证人数的数组
    ...
    def predictAccurateCCC(self, selectcountid, selectdateidx):
        ...
        for i in range(self._windowSizeBegin, self._windowSizeEnd+1):
            try:
                L2Data, popt = self.predict_NRLCQ(selectdateidx, i)
            except:
                return 1, 0, 0, 0, 0, 0
            iList.append(i)
        ...
        L2Data, popt = self.predict_NRLCQ(selectdateidx, xicSelectIDX)
        ...
        if self.judge(list(real_samp_ccc)+list(real_pred_ccc)) == 0 or p >= self._pLimit:
            iserror = 1
        return iserror, L2Data, L2cccData, xicSelectIDX, ape, xicList, popt
    ...
    def predictOneCountry(self, selectcountid):
        ...
        for dateidx in range(idxfirstdate, idxlastdate+1):
            iserror, L2Data, L2cccData, xicSelectIDX, ape, xicList, popt = self.predictAccurateCCC(selectcountid, dateidx)
            ...
            if iserror == 1 or math.isnan(ape) or math.isinf(ape):
                continue
            self._deviationPopt[0].append(abs((self._oripopt[0] - popt[0]) / self._oripopt[0]))
            self._deviationPopt[1].append(abs((self._oripopt[1] - popt[1]) / self._oripopt[1]))
            resultsList.append(ape)
            countryXicList.append(xicList)
        plt.figure()
        plt.subplot(1,1,1)
        mapDateList = [i for i in range(len(resultsList))]
        plt.plot(mapDateList, resultsList, 'r')
        plt.show()
        ...
        poptdev0 = np.array(self._deviationPopt[0])
        poptdev1 = np.array(self._deviationPopt[1])
        print("第一个参数偏差：", np.mean(poptdev0), "±", np.std(poptdev0),end=",")
        print("第二个参数偏差：", np.mean(poptdev1), "±", np.std(poptdev1))
        ...
        with open(self._csvResultsName, 'a+', newline='') as f:
            ...
            data_row = [selectcountid, '', idxfirstdate, idxlastdate, firstdate,
                        ...
                        str(np.mean(resultsArr)) + "&&&" + str(np.std(resultsArr)), resultsArr.size,
                        max(resultsArr),
                        str(np.mean(poptdev0))+"±"+str(np.std(poptdev0)),
                        str(np.mean(poptdev1))+"±"+str(np.std(poptdev1))]
...
def traversalAllRange():
    ...
    for i in range(2,8):
        for j in range(2,8):
            ...
            model.setCsvResultsName("这个是生成的虚拟数据运行结果.csv")
