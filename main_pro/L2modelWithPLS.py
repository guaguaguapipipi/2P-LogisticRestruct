...
import math
import time
# 128Russia,95Malaysia,156Thailand,74Italy,80Korea-South,0Australia,56Germany,162US 测试数据国家选择（八个）
import numpy as np
...
# 这个是5-30的pls,前面加了4个0，方便索引
standPlsList = [0, 0, 0, 0, 0.9993356284516116, 0.9377439825571804, 0.8570078535722114, 0.7699931103465205, 0.690637316500624, 0.6188764358277785, 0.5555728951602056, 0.5037493172890687, 0.45552073810684124, 0.4201164902341754, 0.3848048394106117, 0.36033980678686583, 0.33626487754158035, 0.3147459034968023, 0.2951162637738009, 0.27879342341079183, 0.2641515029239702, 0.2519028307113823, 0.23732001229149902, 0.22654144074090513, 0.2179677898660093, 0.20860741887667808, 0.2018728213822419, 0.1913889374401066, 0.18548960626965544, 0.17817775107519537]
...
class Model:
    ...
    def PLS(self, Y, pred_Y, k):
        plsmodel = PLSRegression(n_components=3)
        xmat = self.x_get(len(Y))
        ymat = np.array([[i] for i in Y])
        plsmodel.fit(xmat, ymat)
        # if np.abs(plsmodel.score(xmat, ymat)) - standPlsList[len(Y)] < 0:
        #     print(np.abs(plsmodel.score(xmat, ymat)) - standPlsList[len(Y)])
        return -(np.abs(plsmodel.score(xmat, ymat)) - standPlsList[len(Y)])
    ...
    def predict_NRLCQ(self, selectIDX, bicIDX):
        ...
        xic = self.PLS(NRLCQ_real_samp, NRLCQ_fit_samp, 2)
    ...
    def getFitCCC(self, lastdateidx, fitNRLCQarr, MIN):
        ...
        fitclist = []
        for i in fitNRLCQarr:
            ccct1 = np.exp(1 - (i * (1 - MIN) + MIN) + np.log(ccct1))
            fitclist.append(ccct1)
        return np.array(fitclist)
...
def traversalAllRange():
    startTime = time.time()
    model = Model("筛选之后的国家.csv", 7)
    for i in range(2,9):
        for j in range(2, 9):
            model.setPlimit(0.01)
            model.setWightMax(i/10)
            model.setDramaticRange(j/10)
            model.setCsvResultsName("这个是PLS筛选之后的国家的运行数据.csv")
